...
for n in i:
    ...
    while True:
        ...
        if position_all:
            ...
            for x in x_frame:
                y = int(A * x ** 2 + B * x + C)
                cv2.circle(display, (x, y), 2, (255, 0, 0), cv2.FILLED)
            ...
            if position[1] < 750:
                if prediction:
                        cvzone.putTextRect(display, "Goes in", (300, 150),
                                        scale=5, thickness=3, colorR=(0, 200, 0), offset=20)
                else:
                        cvzone.putTextRect(display, "OUT", (300, 150),
                                        scale=5, thickness=3, colorR=(0, 0, 200), offset=20)
        #Display
